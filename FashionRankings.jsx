...
export default function FashionRankings() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const saved = await appStorage.load(categorizedBrands, initialRanked);
  ...
  const handleAddBrand = (brandName, categories = ["Clothing"]) => {
  ...
  const fetchedRef = useRef(new Set()); // brandIds we've already auto-fetched this session, to avoid duplicate calls
...
${fitInput}`));
      ...
      const photoUrl = saveFitPhoto ? await appStorage.uploadFitPhoto(saveFitPhoto, id) : null;
...
${profileText}`;
  ...
  // Load cached size chart / price guide when a brand modal opens, and
  // auto-fetch either one that hasn't been looked up before.
  useEffect(() => {
    if (modalBrandId === null) return;
    const b = getBrand(modalBrandId);
    if (!b) return;
    if (fetchedRef.current.has(modalBrandId)) return; // already handled this brand this session
    fetchedRef.current.add(modalBrandId);
    let cancelled = false;
    ...
    (async () => {
      ...
      try {
        ...
        if (cached && !cancelled) {
          setSizeCharts(cached);
          hasSizeChart = true;
        }
      } catch (_) { /* no cache yet */ }
      if (!hasSizeChart && !cancelled) {
        fetchSizeChart(b.name, modalBrandId);
      }

      let hasPriceGuide = false;
      try {
        const cachedP = await appStorage.getPriceGuide(modalBrandId);
        if (cachedP && !cancelled) {
          setPriceGuide(cachedP);
          hasPriceGuide = true;
        }
      } catch (_) { /* no cache yet */ }
      if (!hasPriceGuide && !cancelled) {
        fetchPriceGuide(b.name, modalBrandId);
      }
    })();
  ...
  const fetchSizeChart = async (brandName, brandId) => {
    setSizeChartLoading(true); setSizeChartError(null);
    try {
      const prompt = `Find official women's clothing size charts for the brand "${brandName}". Search if needed to confirm. Include separate charts when the brand has meaningful differences such as dresses vs tops, or US vs international sizing. If exact data isn't findable, provide one reasonable estimated chart and mark it estimated. Respond with ONLY raw JSON, no markdown fences, no other text, in exactly this shape:
    {"charts":[{"label":"US dresses","unit":"in","estimated":true,"sizes":[{"size":"XS","bust":32,"waist":24,"hips":34},{"size":"S","bust":34,"waist":26,"hips":36}],"note":"one short sentence of sizing guidance"}]}`;
      const text = await askClaude(prompt, true);
      const parsed = extractJSON(text);
      const charts = (parsed.charts || []).map((chart) => ({ ...chart, fetchedAt: new Date().toISOString() }));
...
{"verdict":"good fit" or "tight" or "loose" or "unclear","recommendedSize":"e.g. size 4 / M","reasoning":"2-3 sentence explanation referencing the actual numbers"}`;
  ...
  const fetchPriceGuide = async (brandName, brandId) => {
    ...
    try {
      const prompt = `For the clothing brand "${brandName}", give a general sense of typical original retail prices and typical secondhand resale prices (on Poshmark, TheRealReal, ThredUp, or Vestiaire Collective) for a normal piece from this brand (e.g. a dress or top, gently used condition). Search if needed. Respond with ONLY raw JSON, no markdown fences, no other text, in exactly this shape:
{"retailLow":80,"retailHigh":200,"resaleLow":30,"resaleHigh":90,"goodBuyUnder":45,"notes":"1-2 sentence explanation of resale demand/depreciation for this brand"}`;
  ...
  const rankedSearchMatches = ranked.some((entry) => {
    const brand = getBrand(entry.id);
    return brand && brand.name.toLowerCase().includes(normalizedSearch);
  });
  const existingSearchBrand = normalizedSearch
    ? brands.some((brand) => brand.name.toLowerCase().includes(normalizedSearch))
    : false;
    ...
    <div style={{ minHeight: "100vh", width: "100%", boxSizing: "border-box", overflowX: "hidden", background: "#f7f3ed", color: "#302b25", fontFamily: "Georgia, serif", padding: "24px 16px 48px", maxWidth: "920px", margin: "0 auto" }}>
      ...
      {normalizedSearch && !existingSearchBrand && (
        <div style={{ marginBottom: "12px" }}>
          {!addCategoryOpen ? (
            <button onClick={() => setAddCategoryOpen(true)} style={btnStyle(true)}>+ Add and rank brand</button>
          ) : (
            <div style={{ padding: "10px 12px", border: "1px solid #ded6ca", background: "#fffdfa" }}>
              ...
              <button disabled={!newBrandCategories.length} onClick={() => handleAddBrand(search.trim(), newBrandCategories)} style={modalBtnStyle}>Save brand</button>
            </div>
          )}
        </div>
      )}
        ...
        <div>
          {normalizedSearch && !rankedSearchMatches ? null : (
            ...
            const tierBrands = ranked.filter((r) => r.tier === tier.label).map((r) => r.id).filter(id => {
              const b = getBrand(id);
              return b && brandMatchesCategory(b) && (!normalizedSearch || b.name.toLowerCase().includes(normalizedSearch));
            });
        ...
        <div>
          {TIER_CONFIG.map((tier) => {
            const tierBrands = ranked.filter((r) => r.tier === tier.label).filter(r => {
              ...
              return b && brandMatchesCategory(b) && b.name.toLowerCase().includes(search.toLowerCase());
      ...
      {modalBrand && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(33,29,23,0.42)", display: "flex", alignItems: "flex-start", justifyContent: "center", zIndex: 200, padding: "8px", overflowY: "auto", overflowX: "hidden" }} onClick={closeBrandModal}>
          <div style={{ background: "#fffdfa", border: "1px solid #cfc6ba", borderRadius: "0", padding: "18px", width: "100%", boxSizing: "border-box", maxWidth: "560px", margin: "8px 0", boxShadow: "0 16px 48px rgba(49, 39, 28, 0.18)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: "8px" }}>
              ...
              {fitResult && saveFitOpen && (
                <div style={{ marginTop: "12px", padding: "12px 14px", border: "1px solid #ded6ca", background: "#fbf9f5" }}>
                  ...
                  <input type="file" accept="image/*" onChange={(e) => setSaveFitPhoto(e.target.files?.[0] || null)} style={{ width: "100%", marginBottom: "9px", fontFamily: "inherit", fontSize: "11px", color: "#665d53" }} />
            ...
            <div style={{ marginBottom: "8px" }}>
              ...
              {sizeChartError && !sizeCharts.length && (
                <div style={{ display: "flex", alignItems: "center", gap: "8px", marginTop: "6px" }}>
                  <button onClick={() => fetchSizeChart(modalBrand.name, modalBrand.id)} aria-label="Retry size chart" title="Retry size chart" style={iconBtnStyle}>↻</button>
              ...
              {sizeCharts.length > 0 && (() => {
                ...
                return (
                  <div>
                    ...
                    <button onClick={() => fetchSizeChart(modalBrand.name, modalBrand.id)} aria-label="Refresh size chart" title="Refresh size chart" style={{ ...iconBtnStyle, marginTop: "8px" }}>↻</button>
                  </div>
                );
              })()}
            </div>
            ...
            <div style={{ borderTop: "1px solid #ded6ca", marginTop: "22px", paddingTop: "14px" }}>
