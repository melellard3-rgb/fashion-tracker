...
--
-- Scope: UPDATEs public.rankings.tier and .position for ONE account only.
-- No INSERT, no DELETE, no other table touched. Brands, categories, saved fits,
-- profile, and the multi-user setup are left exactly as they are.
--
-- Safety: a row is written only when the brand id AND the brand name both match
-- the source catalog, so a drifted id can never stamp a tier onto the wrong
-- brand. Anything that does not match is reported (grid 2) and left alone.
--
-- Re-runnable: running it twice produces the same result.

begin;

create temporary table bqi_restore (
...
insert into bqi_restore (brand_id, brand_name, tier, position) values
  (119, 'L''AGENCE', 'S', 0),
  (1, 'Alice + Olivia', 'S', 1),
...
--
-- Anything non-zero means the anon key can still write, which would make this
-- restore pointless (it would be wiped again) and is an open security hole.
-- If either number is not 0, STOP and report it before continuing.
-- ---------------------------------------------------------------------------
select
  'canary' as check,
...
-- ---------------------------------------------------------------------------
-- 2. Report everything this will NOT write, before it writes anything.
-- ---------------------------------------------------------------------------
select 'id/name mismatch - SKIPPED' as status, s.brand_id, s.brand_name as source_name, b.name as your_name
from bqi_restore s
...
select 'in source but not in your list', s.brand_id, s.brand_name, null
from bqi_restore s
...
select 'not in initialRanked - LEFT AS-IS, re-rank manually', b.id, b.name, null
from public.brands b
where b.user_id = (select id from auth.users where lower(email) = lower('melellard3@gmail.com'))
  and not exists (select 1 from bqi_restore s where s.brand_id = b.id)
...
-- F will read higher than 7: the 7 original F brands plus every brand
